...
未执行。完成后应生成 `exp/wav2vec2_finetune_1h/`、
`results/predictions/wav2vec2_finetune_1h_dev.jsonl`，并向
`results/metrics.csv` 追加 E3 记录。

## 2026-07-01：E4 WavLM fine-tune 10h

状态：配置完成；启动验证待执行；正式训练未执行。
...
待执行。配置完成时 GPU 正由 `ssl_asr` Python 训练进程占用（约 95%
利用率），为避免干扰当前正式实验，未并行加载或训练 WavLM。

### 正式训练

未执行。完成后应生成 `exp/wavlm_finetune_10h/`、
`results/predictions/wavlm_finetune_10h_dev.jsonl`，并向
`results/metrics.csv` 追加 E4 记录。
